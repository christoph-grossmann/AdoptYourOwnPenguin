\documentclass[9pt]{beamer}

% Use roboto Font (recommended)
\usepackage[sfdefault]{roboto}
%\usepackage[utf8]{inputenc}
%\usepackage[T1]{fontenc}
\usepackage[english]{babel}

% Define where theme files are located. ('/styles')
\usepackage{../flux-beamer-theme/styles/fluxmacros}
\usefolder{../flux-beamer-theme/styles}
% Use Flux theme v0.1 beta
% Available style: asphalt, blue, red, green, gray 
\usetheme[style=asphalt]{flux}

% Extra packages for the demo:
\usepackage{booktabs}
\usepackage{colortbl}
\usepackage{ragged2e}
\usepackage{schemabloc}

\usepackage{xcolor}

\newcommand{\mascot}{../graphics/tux.png}

% Informations
\title{\#AdoptYourOwnPenguin}
\subtitle{Linux Install Party}
\author{Christoph Großmann \& Lea Laux}
%\institute{Ministry of Silly Walks}
\date{\today}
\titlegraphic{\mascot}

\newsavebox\mascotbox

\begin{lrbox}\mascotbox
	\includegraphics[height=3cm]{\mascot}%
\end{lrbox}

\definecolor{background-local}{named}{background}
%\definecolor{Gray-local}{HTML}{858b8f}
%\definecolor{text-local}{HTML}{4F5D66}
%\definecolor{primaryLight-local}{HTML}{34495e}
%\definecolor{primary-local}{HTML}{2d3e50}
%\definecolor{secondary-local}{HTML}{d15306}
%\definecolor{tertiary-local}{HTML}{35837b}

\newcommand{\DarkBG}{\definecolor{background}{named}{primaryLight}}
\newcommand{\ResetBG}{\definecolor{background}{named}{background-local}}

\newcommand{\sectionframe}[1]{%
\DarkBG%
\begin{frame}[plain]%
\section{#1}\centering\Huge\color{background-local}\textbf{#1}%
\vfill\usebox\mascotbox%
\end{frame}%
\ResetBG%
}

\usepackage[most]{tcolorbox}

\tcbset	{
		frame code={}
		center title,
		left=0pt,
		right=0pt,
		top=0pt,
		bottom=0pt,
		colback=primaryLight,
		colframe=white,
		width=.9\linewidth,
		enlarge left by=0mm,
		boxsep=5pt,
		arc=1pt,outer arc=0pt,
		%height=2\baselineskip
		}

\newcommand{\bash}[1]{\begin{tcolorbox}\color{background-local}> \textbf{#1}\end{tcolorbox}}

\begin{document}

	\titlepage
	
	\include{frames/contents}
	
	\sectionframe{Introduction to Linux}
	\include{frames/intro/intro}
	\include{frames/intro/advantages}
	\include{frames/intro/pieces}
	\include{frames/intro/distros}
	
	\sectionframe{A selection of noteworthy distributions}
	\include{frames/distros/debian}
	\include{frames/distros/ubuntu}
	\include{frames/distros/arch}
	
	\sectionframe{Desktop environments}
	\include{frames/desktop/intro}
	\subsection{KDE, MATE, GNOME, Cinnamon, Xfce, and Budgie}%LXQt, and Deepin
	\include{frames/desktop/kde}
	\include{frames/desktop/mate}
	\include{frames/desktop/gnome}
	\include{frames/desktop/cinnamon}
	\include{frames/desktop/xfce}
	\include{frames/desktop/budgie}
	%\include{frames/desktop/lxqt}
	%\include{frames/desktop/deepin}
	
	\sectionframe{Installing and working with a Linux system}
	\include{frames/install/install}
	\include{frames/install/basics}
	\include{frames/install/software}
	\include{frames/install/trouble}
	
	%\include{frames/refs/lof}
	\include{frames/refs/refs}
	\include{frames/refs/flux}
	\include{frames/refs/selfref}
	
	\include{frames/end}

\end{document}
